    ...
    for (const lib of manifest.libraries) {
        ...
        if (lib.bundle) {
            ...
            files.push({
                ...
                bundler: bundle.bundler || (bundle.browserify ? 'browserify' : 'esbuild'),
                esbuild: bundle.esbuild,
                browserify: bundle.browserify,
                require: bundle.require || null,
                exportName: bundle.exportName || null,
...
async function extractNpmPackage(npm, version, destDir) {
    ...
    execFileSync('tar', ['-xzf', packed], { cwd: destDir, shell: SHELL });
...
async function vendorBundle(file, force) {
    ...
    try {
        execFileSync(NPM, ['init', '-y'], { cwd: work, stdio: 'ignore', shell: SHELL });
        ...
        await mkdir(dirname(destAbs), { recursive: true });
        if (file.bundler === 'browserify') {
            const args = ['--yes', `browserify@${file.browserify}`];
            if (file.exportName) {
                const shim = join(work, 'vendor-entry.cjs');
                await writeFile(
                    shim,
                    `module.exports = require('${file.npm}').${file.exportName};\n`,
                );
                args.push(shim, '-s', file.globalName, '-o', destAbs);
            } else if (file.require) {
                args.push('-r', file.require, '-s', file.globalName, '-o', destAbs);
            } else {
                const entry = join(work, 'node_modules', file.npm, file.entry);
                args.push(entry, '-s', file.globalName, '-o', destAbs);
            }
            execFileSync(NPX, args, { cwd: work, stdio: 'inherit', shell: SHELL });
        } else {
            const entry = join(work, 'node_modules', file.npm, file.entry);
            execFileSync(
                NPX,
                ...
                { cwd: work, stdio: 'inherit', shell: SHELL },
            );
        }
        const buf = await readFile(destAbs);
        file.source.sha256 = sha256Buffer(buf);
